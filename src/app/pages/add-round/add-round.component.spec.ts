...
describe('AddRoundPage', () => {
  ...
  it('defaults the date field to today', () => {
    expect(component.roundForm.controls.date.value).toBe(component.todayIsoDate);
    expect(component.formattedDateLabel).toContain(component.todayIsoDate.slice(8, 10));
  });

  it('keeps the add-tee action disabled until a course is selected', () => {
    expect(component.isAddTeeDisabled).toBe(true);

    component.roundForm.controls.courseId.setValue('course-1');

    expect(component.isAddTeeDisabled).toBe(false);
  });
  ...
  it('auto-selects a newly created course and tee without clearing other fields', async () => {
    ...
    await component.onCourseCreated({
  ...
  it('rejects future dates', () => {
    component.roundForm.controls.date.setValue('2999-01-01');

    expect(component.roundForm.controls.date.hasError('futureDate')).toBe(true);
  ...
  it('enforces gross score limits', () => {
    component.roundForm.controls.grossScore.setValue('19');
    expect(component.roundForm.controls.grossScore.hasError('min')).toBe(true);

    component.roundForm.controls.grossScore.setValue('301');
    expect(component.roundForm.controls.grossScore.hasError('max')).toBe(true);
  });
});
